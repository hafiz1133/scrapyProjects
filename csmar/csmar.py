...
class CSMAR(scrapy.Spider):
    ...
    headers = {'Host': 'ims.csmaor.com', 'Connection': 'keep-alive', 'Content-Length': '261', 'Cache-Control': 'max-age=0', 'Upgrade-Insecure-Requests': '1', 'Origin': 'https://ims.csmaor.com', 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.183 Safari/537.36', 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'Sec-Fetch-Site': 'same-origin', 'Sec-Fetch-Mode': 'navigate', 'Sec-Fetch-User': '?1', 'Sec-Fetch-Dest': 'iframe', 'Referer': 'https://ims.csmaor.com/scripts/mgrqispi.dll?APPNAME=IMS&PRGNAME=IMSMemberLogin&ARGUMENTS=-ACVAR&SessionType=N&ServiceName=OSRH&NotLogin=Y', 'Accept-Encoding': 'gzip, deflate, br', 'Accept-Language': 'en-US,en;q=0.9', 'Cookie': '_ga=GA1.2.98921885.1604987791; _gid=GA1.2.1464911120.1604987791; imssession=524085313; imslastConnect=1604989798473'}
    body = 'APPNAME=IMS&PRGNAME=IMSAgentsandOffices&ARGUMENTS=-A565523330%2C-N2%2CLastName%2C%28P%29+Agent+Nickname%2C%28P%29+Office+Name%2C%28P%29+City%2C%28P%29+Zip+Code&Search_Type=A&LastName={}&%28P%29+Agent+Nickname=&%28P%29+Office+Name=&%28P%29+City=&%28P%29+Zip+Code='
    ...
    def parse_agent(self, response):
        ...
        for tr in response.css('.table.table-striped tr'):
            if 'Phone Number' in tr.css('td ::Text').extract()[0]:
                item['phone number'] = '1- {}'.format(' '.join([v.strip() for v in tr.css('td ::Text').extract()[1:] if v.strip()]))
            if 'Office Phone' in tr.css('td ::Text').extract()[0]:
                item['office number'] = '1- {}'.format(' '.join([v.strip() for v in tr.css('td ::Text').extract()[1:] if v.strip()]))   #' '.join([v.strip() for v in tr.css('td ::Text').extract()[1:] if v.strip()])
